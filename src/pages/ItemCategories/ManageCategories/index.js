...
const ManageItemCategories = () => {
  const { categories: arraycat, salonCategories } = useSelector(
    (state) => state.branch
  );
  ...
  const currSalonId = salonId || undefined;
  ...
  const getAllData = (selectedBranch) => {
    if (isSalonAdmin) {
      dispatch(getSalonCategories(salonId));
    }
    if (isBranchAdmin) {
      dispatch(getBranchCategories(salonId, branchId));
    }
    if (isSuperAdmin) {
  ...
  const importData = () => {
    if (isSalonAdmin) {
      return categoryTypes.filter((item) => {
        return (
          salonCategories.findIndex((data) => {
    ...
    if (isBranchAdmin) {
      return salonCategories.filter((item) => {
        return (
  ...
  React.useEffect(() => {
    ...
    isSalonAdmin && dispatch(getAllBranches(salonId));
  }, [selectedBranch]);
